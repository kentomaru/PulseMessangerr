...
import type { ActiveCall, CallPayload, PublicUser } from "./types";
...
export function useCallController(notify: ToastFn) {
  ...
  const startCall = useCallback(
    async (conversationId: string, peer: PublicUser) => {
      ...
      try {
        ...
        syncCall({ id: created.id, conversationId, role: "caller", phase: "outgoing", peer });
  ...
  const accept = useCallback(async () => {
    ...
    const peer = inc.caller as PublicUser;
    syncCall({
      id: inc.id,
